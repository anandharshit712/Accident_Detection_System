...
# dense_autoencoder_model = load_model('dense_autoencoder_model.keras')
...
def calculate_dynamic_weights(isolation_forest_score, autoencoder_score):
    """
    Dynamically calculate weights for the models based on their scores.
    Scores are normalized to sum up to 1.
    """
    total_score = isolation_forest_score + autoencoder_score
    if total_score == 0:
        return 0.5, 0.5  # Equal weights if both scores are zero
    isolation_forest_weight = isolation_forest_score / total_score
    autoencoder_weight = autoencoder_score / total_score
    return isolation_forest_weight, autoencoder_weight
...
def process_data_and_save_json():
    ...
    for mobile_number, data_entries in mobile_number_groups.items():
        ...
        # Step 2: Use Dense Autoencoder for anomaly detection
        autoencoder_reconstruction = dense_autoencoder_model.predict(data_array)
        autoencoder_score = np.mean(np.square(data_array - autoencoder_reconstruction))

        # Step 3: Calculate dynamic weights
        isolation_forest_weight, autoencoder_weight = calculate_dynamic_weights(isolation_forest_score, autoencoder_score)

        # Step 4: Calculate weighted anomaly score
        raw_anomaly_score = (isolation_forest_weight * isolation_forest_score) + (autoencoder_weight * autoencoder_score)
        ...
        anomaly_scores_map[mobile_number] = {
            ...
            "weights": {
                ...
                "Autoencoder Weight": autoencoder_weight
            }
        }
    ...
    for mobile_number, data in anomaly_scores_map.items():
        ...
        if is_anomaly:  # Save only true anomalies
            accident_details = {
                ...
                "lat": data["latest_data"]['latitude'],
                "long": data["latest_data"]['longitude']
                ,
                "status": True
            }
...
print("Data send to accident database!!!!")
